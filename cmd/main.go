...
// refresherRunnable adapts config.Refresher to manager.Runnable so the
// manager owns its lifecycle (and waits for caches before starting).
type refresherRunnable struct{ r *config.Refresher }

func (rr refresherRunnable) Start(ctx context.Context) error { return rr.r.Start(ctx) }
...
func main() {
	...
	flag.StringVar(&configName, "config-name", "workload-resizer-config",
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "bc256639.workload-resizer.io",
	...
	refresher := &config.Refresher{
		...
		Name:      configName,
		Interval:  configRefreshInterval,
	}
	if err := mgr.Add(refresherRunnable{r: refresher}); err != nil {
		setupLog.Error(err, "Failed to register config refresher")
		os.Exit(1)
	}
	...
	if err := (&controller.PodReconciler{
		...
		// GetEventRecorderFor is deprecated in favor of the v1 events API;
